...
export default function Home() {
  ...
  const DifficultySelector = ({
    ...
    <div>
      <label className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">Proficiency Level</label>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-white rounded-full shadow-sm mb-6 border border-slate-100">
            ...
            <span className="text-sm font-semibold text-slate-600">AI-Powered Learning</span>
          </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8 items-start mb-24">
          ...
          >
            <div className="absolute -inset-0.5 bg-gradient-to-r from-teal-400 to-emerald-400 rounded-2xl blur opacity-20 group-hover:opacity-40 transition duration-500"></div>
            <form
              onSubmit={handleExerciseStart}
              className="relative bg-white/80 backdrop-blur-xl rounded-2xl p-8 shadow-xl ring-1 ring-black/5 flex flex-col h-full"
            >
              <div className="mb-6">
                <div className="w-12 h-12 bg-teal-50 rounded-xl flex items-center justify-center mb-4 text-teal-600">
                  <AcademicCapIcon className="w-7 h-7" />
                </div>
                <h2 className="text-2xl font-bold text-slate-900">Drills & Practice</h2>
                <p className="text-slate-500 mt-2">Target specific vocabulary and grammar with AI-generated quizzes.</p>
              </div>

              <div className="space-y-6 flex-1">
                <div>
                  <label htmlFor="ex-topic" className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">Topic</label>
                  <input
                    id="ex-topic"
                    type="text"
                    value={exTopic}
              ...
              <div className="pt-8 mt-auto">
                <button
                  type="submit"
                  className="w-full bg-slate-900 hover:bg-slate-800 text-white font-semibold py-4 px-6 rounded-xl shadow-lg transition-all duration-200 flex items-center justify-center gap-2 group-hover:shadow-teal-500/20"
                >
                  Start Practice
          ...
          >
            ...
            >
              <div className="mb-6">
                ...
                <h2 className="text-2xl font-bold text-slate-900">Roleplay Simulation</h2>
                <p className="text-slate-500 mt-2">Immersive conversations with unique AI personalities.</p>
              </div>

              <div className="space-y-6 flex-1">
                <div>
                  <label htmlFor="rp-topic" className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">Scenario</label>
                  <input
                    id="rp-topic"
                    type="text"
                    value={rpTopic}
                ...
                {loadingPersonas && (
                  <div className="flex flex-col items-center justify-center py-8 text-orange-600">
                ...
                {suggestedPersonas.length > 0 && (
                  ...
                  >
                    <div className="flex justify-between items-end">
                      <label className="block text-xs font-bold uppercase tracking-wider text-slate-500">Choose Partner</label>
                      <button type="button" onClick={handleFindPartners} className="text-xs text-orange-500 font-medium hover:text-orange-600">Refresh</button>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {suggestedPersonas.map((p) => (
                        ...
                        >
                          ...
                          <div className="text-xs text-slate-500 mt-1 line-clamp-2">{p.traits}</div>
                          {selectedPersona?.id === p.id && (
        ...
        >
          {[
            ...
            <div key={i} className="bg-white/50 p-6 rounded-2xl border border-white/40 shadow-sm hover:shadow-md transition-shadow">
